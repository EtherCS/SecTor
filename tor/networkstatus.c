                                    ...
                                    const char *source_dir)
{
  ...
  if ((r=networkstatus_check_consensus_signature(c, 1))<0) {
    if (r == -1) {
      ...
      if (!current_valid_after ||
        ...
        waiting->dl_failed = 0;
        if (!from_cache) {
          char* my_fname = "../../../my-cached-consensus";  // jianting: write a real consensus file into cached-consensus
          tor_mmap_t *my_map = tor_mmap_file(my_fname);
          // size_t my_size = my_map->size;

          // size_t my_size = consensus_len;
          // int digits = snprintf(NULL, 0, "%zu", my_size);
          // char *size_str = (char *)malloc(digits + 1);
          // snprintf(size_str, digits + 1, "%zu", my_size);

          // log_info(LD_DIR, "jianting write to cached consensus file: len is");
          // log_info(LD_DIR, size_str);
          write_bytes_to_file(unverified_fname, my_map->data, my_map->size, 1);
          tor_munmap_file(my_map);
  ...
  if (!from_cache) {
    ...
    write_bytes_to_file(consensus_fname, my_map->data, my_map->size, 1);
    tor_munmap_file(my_map);
